...
# https://pae-paha.pacioos.hawaii.edu/erddap/griddap/dhw_5km.geotif?CRW_SST%5B(2020-11-15T12:00:00Z):1:(2020-11-15T12:00:00Z)%5D%5B(89.975):1:(-89.975)%5D%5B(-179.975):1:(179.975)%5D
...
def request(datetime):
    url = f'https://pae-paha.pacioos.hawaii.edu/erddap/griddap/dhw_5km.geotif?CRW_SST%5B({datetime}):1:({datetime})%5D%5B(89.975):1:(-89.975)%5D%5B(-179.975):1:(179.975)%5D'
...
def stac(filename, datetime):
    ...
    data["assets"]["image"]["href"] = f"s3://covariate-ingest-data-dev/sst/cogs/{idstring}.tif"
    data["assets"]["image"]["href"] = f"s3://covariate-ingest-data-dev/sst/cogs/{idstring}.tif"
    data["assets"]["thumbnail"]["href"] = f"s3://covariate-ingest-data-dev/sst/cogs/{idstring}.tif"
    data["links"][0] = {
        "rel":"self",
        "href":f"s3://covariate-ingest-data-dev/sst/stac_items/{idstring}.json"
    }
    data["links"][1] = {
        "rel":"collection",
        "href":f"s3://covariate-ingest-data-dev/sst/collection.json"
    }
    data["links"][2] = {
        "rel":"parent",
        "href":f"s3://covariate-ingest-data-dev/sst/collection.json"
...
def to_aws(cog_file, filename, stac_item): 
    filename = f"sst/cogs/cog_{filename}"
    stac_aws = f"sst/{stac_item}"
